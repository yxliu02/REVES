...
from utils.ab_mcts_m.algo import ABMCTSM           # or: from treequest.algos.ab_mcts_m.algo import ABMCTSM
...
class AB_MCTS_MPlanner(Planner):
    """
    Wrapper planner that uses ABMCTSM internally.
    Same semantics as your MCTSPlanner: accumulate reflection text along the path,
    evaluate each answer, and keep the best raw answer seen so far.
    """
    ...
    def __init__(
        ...
        # ABMCTSM knobs (match your algo constructor)
        ...
        # Instantiate ABMCTSM with your settings
        self._algo = ABMCTSM(
    ...
    def run(self, query: Dict[str, Any], reference_info: Optional[str] = None) -> str:
        ...
        # Provide action(s) for ABMCTSM.
        # Single action works; ABMCTSM will still choose between existing children vs. GEN (new child).
        ...
        for _ in range(self.simulations):
            ...
            if best_score == 0.0:
                return best_raw, self.usage, _ + 1  # type: ignore[return-value]

        # Return the best plan encountered
        return best_raw, self.usage, self.simulations  # type: ignore[return-value]
